<?php
	...
	# ----------------------------------------------------------------------------------------------------
		 ...
		 * (see mysql docs)
		 */
		function getRowCount($table, $maindb=SYSTEMDB){
			$sql = "SELECT COUNT(*) as total FROM $table";
			$db = db_getDBObject($maindb);
			if($r = $db->query($sql)){
		...
		function getRowCountSQL($sql, $maindb=SYSTEMDB){
			$db = db_getDBObject($maindb);
			if($r = $db->query($sql)){
	...
	# ----------------------------------------------------------------------------------------------------
		function _handle_error($query,$db_debug=0) {
			...
			if ($db_debug) {
				...
				echo "Ошибка Базы Данных. Системный администратор был уведомлен, и эта проблема будет решена в кратчайшие сроки. Приносим свои извинения за неудобства.";
		...
		function _mymail($to,$subject,$message,$from,$xheaders="") {
			$eMailerObj = new EMailer($to, $subject, $message, $from);
			if ($xheaders) $eMailerObj->setExtraHeaders($xheaders);
			return $eMailerObj->send();
		}
